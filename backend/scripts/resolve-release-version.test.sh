#!/usr/bin/env bash
# Parser unit tests for resolve_release_version.
# Sources resolve-release-version.sh to get the function only.
# NEVER makes network or SSH calls.

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"

# Source the resolver to pick up resolve_release_version().
# shellcheck source=resolve-release-version.sh
. "$SCRIPT_DIR/resolve-release-version.sh"
...
assert_success() {
  ...
  local actual
  actual="$(resolve_release_version "$input" 2>/dev/null)"
  local rc=$?
